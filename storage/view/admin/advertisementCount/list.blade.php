            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div id="example2_wrapper" class="dataTables_wrapper dt-bootstrap4">
                        <div class="row">
                            <form action="/admin/advertisement_count/list" method="get" class="form-inline" enctype="multipart/form-data">
                                ...
                                <div class="form-group mx-1">
                                    <label for="exampleInputEmail1" class="mx-1">{{trans('default.ad_control.ad_name') ?? '廣告名稱'}}</label>
                                    <input type="text" class="form-control" name="advertisements_name" placeholder="{{trans('default.ad_control.ad_input_name') ?? '廣告名稱'}}" value="{{$advertisements_name ?? ''}}">
                                </div>
                                ...
                                <table id="example2" class="table table-bordered table-hover dataTable dtr-inline"
                                    ...
                                    <tr>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="Engine version: activate to sort column ascending">{{trans('default.ad_control.ad_name') ?? '廣告名稱'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                    ...
                                    @foreach($datas as $key => $model)
                                        <tr class="odd">
                                            ...
                                            <td>{{ $model->advertisement_name}}</td>
                                            <td>{{ $model->click_date}}</td>
                                            <td>{{ $model->click_num}}</td>
                                            <td>{{ $model->site_name ?? trans('default.unattribution_web')}}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                    <tfoot>
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1">{{trans('default.ad_control.ad_name') ?? '廣告名稱'}}</th>
