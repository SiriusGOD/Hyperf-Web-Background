...
if (! function_exists('hasPermission')) {
    function hasPermission(array $callbacks){
...
if (! function_exists('authPermission')) {
    function authPermission(string $key)
    {
        ...
        if (redis()->exists($redisKey)) {
            $perArys = json_decode(redis()->get($redisKey), true);
        } else {
            $service = di()->get(App\Service\PermissionService::class);
            $perArys = $service->getUserPermission($role_id);
            // save permission to redis
            redis()->set($redisKey, json_encode($perArys));
        }
        return in_array($key, $perArys) ? true : false;
    }
}

if (! function_exists('resetPermission')) {
...
if (! function_exists('cutStrLang')) {
    function cutStrLang(string $str): string
    {
        $collect = explode('-', $str);
...
if (! function_exists('allowIp')) {
    function allowIp($ip)
    {
        ...
        if (ipInArray(md5($ip), explode(',', $allowIpTxts)) || ipInArray($ip, $allowIps)) {
            setLog()->info('md5 or ipInArray true ');
            return true;
        }
        setLog()->info(date('ymd') . ' - deny ip = ' . $ip);
        return false;
    }
}
